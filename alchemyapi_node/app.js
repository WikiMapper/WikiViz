...
server.listen(port, function(){
	console.log('Express server listening on port ' + port + ' (localhost:3000)');
});
...
function keywords(output) {
	alchemyapi.keywords('url', 'http://en.wikipedia.org/wiki/Strangers_with_Candy_(film)', { 'sentiment':1 }, function(response) {
		output['keywords'] = { text:demo_text, response:JSON.stringify(response,null,4), results:response['keywords'] };
		// concepts(req, res, output);
		for (var i in output['keywords']){
			console.log(output['keywords'][i]['text']);
		}
	});
}
